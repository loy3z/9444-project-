...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fine-tune and evaluate ResNet50 on EuroSAT RGB.",
    ...
    parser.add_argument(
        "--no-pretrained",
        action="store_true",
        help="Do not load ImageNet weights (not recommended for the baseline).",
    )
    parser.add_argument(
        "--freeze-backbone",
        action="store_true",
    ...
    if args.output_dir is None:
        if args.smoke_test:
            experiment_name = "resnet50_smoke"
        elif args.class_weighted:
            experiment_name = "resnet50_class_weighted"
        else:
...
def main() -> None:
    ...
    print("=" * 72)
    print("COMP9444 Project 54 - ResNet50")
    print("=" * 72)
